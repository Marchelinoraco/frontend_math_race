  ...
  const deleteData = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:4000/siswa/${uuidStudent}`
      );
      setData(data.filter((item) => item.uuid !== response.data.student.uuid));
      setOpenModal(false);
    } catch (error) {
      console.log(error);
    }
    ...
    <>
      <div>
        <div className="flex justify-end items-center">
          <Link
            to="../add-student"
            className="bg-[#EAC301] py-2 px-4 text-white rounded-lg "
          >
            Tambah Siswa
          </Link>
        </div>
            ...
            <tbody className="bg-white">
              {data.map((item, number) => (
                <tr key={number + 1}>
                  ...
                  <td>
                    <button
                      className="py-1 px-4 text-white rounded-lg  bg-red-600"
